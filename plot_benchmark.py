    ...
    for eps in [0, 0.1, 0.01, 0.001, 0.0001]:
        for x in [100, 200, 300]:
            ...
            if gr != zzz:
                ...
                data = open(
                    filename + "/benchmark/qtg/ub=no/bias=300.000000/M=200/states=comp/runtime.txt").read().split("\n")
                data.pop()
...
plt.errorbar([i for i in range(len(runtime))], runtime, fmt=".--", yerr=runtime_std, label="classical")
plt.errorbar([i for i in range(len(qruntime))], qruntime, fmt=".--", yerr=qruntime_std, label="quantum")
x, y = plt.ylim()
plt.vlines(b, x, y, linestyles="--", colors="red", linewidth=1)
plt.ylim(x, y)

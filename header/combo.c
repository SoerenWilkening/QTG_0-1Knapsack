...
stype combo(item *f, item *l, stype c, stype lb, stype ub,
            boolean def, boolean relx, int *out);
...
static void definesolution(allinfo *a, int *out) {
    register item *i, *h, *m;
    ...
    if (ws == wsum) {
        ...
        combo(f, l, wsum, psum - 1, psum, TRUE, TRUE, out);
    }
}
...
static void solvesur(allinfo *a, item *f, item *l, stype minsur, stype maxsur,
    ...
    z1 = ps + combo(k, l, csur, a->z - ps, 0, TRUE, TRUE, NULL);

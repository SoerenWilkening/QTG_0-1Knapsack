...
#define MAXSTATES 18000000    /* adapted for bigger instances */
...
/* interval record */
typedef struct {
    combo_item *f;               /* first item in interval  */
    combo_item *l;               /* last item in interval   */
} interval;
...
typedef struct pset {
    ...
    combo_item *vitem[MAXV];  /* current last MAXV items */
    combo_item *ovitem[MAXV]; /* optimal set of items    */
    btype ovect;         /* optimal solution vector */
} partset;


typedef struct { /* all info for solving separated problem */
    combo_item *fitem;        /* first item in problem */
    combo_item *litem;        /* last item in problem */
    combo_item *s;            /* core is [s,t] */
    combo_item *t;
    combo_item *b;            /* break item */
    combo_item *fpart;        /* first element in sorted core */
    combo_item *lpart;        /* last element in sorted core */
    ...
    combo_item *fsol;         /* first item in opt solution (fullsol=FALSE) */
    combo_item *lsol;         /* last item in opt solution (fullsol=FALSE) */
    combo_item *ffull;        /* start of item array (fullsol=TRUE) */
    combo_item *lfull;        /* end of item array (fullsol=TRUE) */
...
static void push(allinfo *a, int side, combo_item *f, combo_item *l) {
    interval *pos;
    switch (side) {
        case LEFT :
            pos = a->intv1;
...
static void pop(allinfo *a, int side, combo_item **f, combo_item **l) {
    interval *pos;
    switch (side) {
        case LEFT :
...
static void improvesol(allinfo *a, state *v) {
    ...
    a->d.ovect = v->vect;
    a->fullsol = FALSE;
    memcpy(a->d.ovitem, a->d.vitem, sizeof(combo_item *) * MAXV);
}
...
static void partsort(allinfo *a, combo_item *f, combo_item *l, stype ws, stype c, int what) {
...
static combo_item *minweights(combo_item *f, combo_item *l, stype c) {
    register itype mw;
    register combo_item *i, *j, *m;
    register stype ws;
    int d;
...
static combo_item *maxprofits(combo_item *f, combo_item *l, stype z) {
    register itype mp;
    register combo_item *i, *j, *m;
    register stype ps;
    int d;
...
static void sursort(combo_item *f, combo_item *l, itype sur, stype c,
                    stype *p1, stype *w1, combo_item **b) {
    register itype s;
    register prod mp, mw;
    register combo_item *i, *j, *m;
...
static boolean haschance(allinfo *a, combo_item *i, int side) {
    register itype p, w;
    register state *j, *m;
...
static void multiply(allinfo *a, combo_item *h, int side) {
...
static void surbin(combo_item *f, combo_item *l, itype s1, itype s2, stype c,
...
static void solvesur(allinfo *a, combo_item *f, combo_item *l, stype minsur, stype maxsur,
                     ntype card, stype *ub) {
    register combo_item *i, *k, *m;
...
static void simpreduce(int side, combo_item **f, combo_item **l, allinfo *a) {
    register combo_item *i, *j, *k;
    register prod pb, wb;
    register prod q;
    register int red;
...
static void initfirst(allinfo *a, stype pb, stype wb) {
    ...
    /* init full solution */
    a->fullsol = FALSE;
    a->ffull = palloc(DIFF(a->fitem, a->litem), sizeof(combo_item));
...
static void swapout(allinfo *a, combo_item *i, int side) {
    interval *pos, *k;
